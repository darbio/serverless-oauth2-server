...
export class CallbackHandler extends Handler {
    async get(event : APIGatewayProxyEvent, context : Context, callback : Callback < APIGatewayProxyResult >) {
        try {
            // Parameters
            ...
            let provider = await providerRepository.get(providerSession.provider);
            ...
            let tokenRequest = await request.post(provider.tokenUrl, requestOptions);
            ...
            if (!user.hasIdentityFromExternalProvider({provider: provider.id})) {
                user.addExternalIdentity(ExternalIdentity.create({provider: provider.id, sub: sub}));
                await userRepository.save(user);
            }

            // Get the original session
            let sessionRepository : ISessionRepository = new SessionRepository();
            let session = await sessionRepository.get(providerSession.sessionId);

            // Generate an authorization code
            const code = AuthorizationCode.create({subject: username, clientId: session.clientId, redirectUrl: session.redirectUri});
