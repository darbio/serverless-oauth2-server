    ...
    try {
        ...
        switch (grant_type) {
            case "authorization_code":
                ...
                if (!validator.isURL(redirect_uri)) {
                    callback(null, {
                        ...
                        body: JSON.stringify({
                            error: "invalid_grant",
                            error_description:
                                "The redirect uri code is invalid."
                        })
                    });
                    return;
                }
                ...
                const authorizationCode = await authorizationCodeRepository.get(
                    code
                );

                if (
                    authorizationCode === null ||
                    !authorizationCode.isValid()
                ) {
                    callback(null, {
                        statusCode: 400,
                        body: JSON.stringify({
                ...
                if (client_id !== authorizationCode.clientId) {
                    callback(null, {
                        ...
                        body: JSON.stringify({
                            error: "invalid_client",
                            error_description:
                                "Request contains an invalid client id."
                        })
                    });
                    return;
                }
